...
description: >
  ...
  Arbeitsverzeichnis. Downstream-Skill von sxo-analyze und sxo-wireframe.
  ...
  Do NOT use for: wireframe creation (use sxo-wireframe), SXO analysis
  (use sxo-analyze), blog posts without SXO context (use blog-write),
...
2. SXO-Wireframe: sxo-wireframe-*.html  (aus /sxo-wireframe)
...
IF nur Report ODER nur Wireframe:
  ...
  -> "Report fehlt? -> /sxo-analyze [keyword] [url]"
  -> "Wireframe fehlt? -> /sxo-wireframe [report-datei]"
  -> Falls nur Wireframe: Fahre trotzdem fort (Wireframe enthaelt User Story + Gaps)
  -> Falls nur Report: Abbruch mit Empfehlung /sxo-wireframe zuerst

IF weder Report noch Wireframe:
  -> Frage nach Dateien oder empfehle /sxo-analyze als Startpunkt
```
...
IF Wireframe-Datei nicht parsbar:
  ...
  -> Biete an: "/sxo-wireframe [report] fuer einen neuen Wireframe"
